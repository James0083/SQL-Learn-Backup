...
select sal from emp swhere ename='SCOTT';
...
[4] EXIST 연산자 : 서브쿼리의 데이터가 존재하는지 여부를 따져서 존재하는 값들만 반환한다
...
SELECT * FROM MEMBER
WHERE MILEAGE > ANY(SELECT MILEAGE FROM MEMBER);
...
SELECT ENAME, JOB, DNAME, LOC
FROM 
(SELECT * FROM EMP WHERE JOB='MANAGER') A JOIN DEPT E
...
--기존 데이터에 ROWNUM을 먼저 붙이고 그런 뒤에 ORDER BY 를 한다
--이런 문제를 해결하기 위해 ORDER BY를 먼저 하게끔 서브쿼리를 사용해보자
SELECT * FROM (
